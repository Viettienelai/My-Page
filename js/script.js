...
window.addEventListener('scroll', () => {
    ...
    // Nếu vị trí cuộn nằm trong khoảng từ 0 đến 300px, di chuyển container lên, Nếu vị trí cuộn lớn hơn 300px, giữ container ở vị trí cố định
    if (scrollTop <= 300) { earchBarContainer.style.top = 290 - scrollTop + 'px';} else { searchBarContainer.style.top = '-10px'; }

    // Tính toán tỷ lệ thay đổi hiệu ứng 1 (từ 0 đến 1)
    const progress1 = Math.min(scrollTop / 300, 1);

    // Cập nhật border-radius, width và height của bar dựa trên progress1
    ...
    if (scrollTop >= 260) { progress2 = Math.min((scrollTop - 260) / 40, 1); }
